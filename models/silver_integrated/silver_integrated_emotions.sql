...
aggregations AS (
    SELECT
        ...
        AVG(emotion_suprise) AS emotion_suprise,
...
SELECT
    ...
    ROUND(COALESCE(emotion_suprise, LAST_VALUE(emotion_suprise IGNORE NULLS) OVER (PARTITION BY participation_id ORDER BY integrated_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)), 3) AS emotion_suprise,
